...
class IdlingComponent{
	...
	public function checkAndSetIdling() : bool{
		...
		if(!$this->idling and // of course it should not idle currently
			...
			if(\kenygamer\Core\Main::mt_rand(0, 100) <= $this->idleChance){ // with a chance of x percent the entity starts to idle
				$this->idling = true;
				$this->idlingTickCounter = new TickCounter(\kenygamer\Core\Main::mt_rand($this->idleMin, $this->idleMax)); // idle x and x ticks
	...
	public function doSomeIdleStuff(int $tickDiff = 1){
		if($this->idlingCounter->isTicksExpired($tickDiff)){
			...
			$yaw = \kenygamer\Core\Main::mt_rand(0, 1) ? $yaw + \kenygamer\Core\Main::mt_rand(15, 45) : $yaw - \kenygamer\Core\Main::mt_rand(15, 45);
			if($yaw > 360){
				$yaw = 360;
			}else if($yaw < 0){
				$yaw = 0;
			...
			// completely ok
			$pitch = \kenygamer\Core\Main::mt_rand(0, 1) ? $pitch + \kenygamer\Core\Main::mt_rand(10, 20) : $pitch - \kenygamer\Core\Main::mt_rand(10, 20);
			if($pitch > 60){
				$pitch = 60;
			}else if($pitch < -60){
				$pitch = -60;
