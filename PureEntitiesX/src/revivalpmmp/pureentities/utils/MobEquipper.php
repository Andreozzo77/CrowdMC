...
class MobEquipper{
	...
	public static function equipMob(BaseEntity $entity){
		if($entity instanceof IntfCanEquip){
			...
			$wearWeapon = (\kenygamer\Core\Main::mt_rand(0, 100) <= $wearPickupChance->getWeaponChance());
			$wearArmor = (\kenygamer\Core\Main::mt_rand(0, 100) <= $wearPickupChance->getArmorChance());

			if($wearWeapon){
				// 1/3 chance of iron sword, 2/3 iron shovel
				$weaponItem = Item::get((\kenygamer\Core\Main::mt_rand(0, 3) <= 1) ? ItemIds::IRON_SWORD : ItemIds::IRON_SHOVEL);
			...
			if($wearArmor){
				...
				if(\kenygamer\Core\Main::mt_rand(0, 100) <= $wearChances->getFullChance()){ // full armor
				...
				}else if(\kenygamer\Core\Main::mt_rand(0, 100) <= $wearChances->getHelmetChestplateLeggingsChance()){ // all without boots
					$entity->getMobEquipment()->setHelmet(self::getHelmet($armorType));
					$entity->getMobEquipment()->setLeggings(self::getLeggings($armorType));
					$entity->getMobEquipment()->setChestplate(self::getChestplate($armorType));
					PureEntities::logOutput("Helmet, leggings and chestplate of $armorType for $entity", PureEntities::DEBUG);
				}else if(\kenygamer\Core\Main::mt_rand(0, 100) <= $wearChances->getHelmetChestplateChance()){ // only helmet and chestplate
					$entity->getMobEquipment()->setHelmet(self::getHelmet($armorType));
					$entity->getMobEquipment()->setChestplate(self::getChestplate($armorType));
					PureEntities::logOutput("Helmet and chestplate of type $armorType for $entity", PureEntities::DEBUG);
				}else if(\kenygamer\Core\Main::mt_rand(0, 100) <= $wearChances->getHelmetChance()){ // only helmet
					$entity->getMobEquipment()->setHelmet(self::getHelmet($armorType));
	...
	private static function getArmorType(ArmorTypeChances $chances) : string{
		if(\kenygamer\Core\Main::mt_rand(0, 100) <= $chances->getDiamond()){
			return self::DIAMOND;
		}else if(\kenygamer\Core\Main::mt_rand(0, 100) <= $chances->getGold()){
			return self::GOLD;
		}else if(\kenygamer\Core\Main::mt_rand(0, 100) <= $chances->getIron()){
			return self::IRON;
		}else if(\kenygamer\Core\Main::mt_rand(0, 100) <= $chances->getLeather()){
			return self::LEATHER;
		}else{
