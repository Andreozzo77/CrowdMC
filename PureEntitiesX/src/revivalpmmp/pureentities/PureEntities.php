...
class PureEntities extends PluginBase implements CommandExecutor{
	...
	public function onEnable(){
		...
		if($enabled){
			$level = self::$loglevel = mb_strtolower($this->getConfig()->getNested("logfile.loglevel", self::NORM));
		}
	}
		...
		switch($command->getName()){
				...
				if(count($args) <= 1){
					...
					foreach(Server::getInstance()->getLevels() as $level){
						foreach($level->getEntities() as $entity){
							...
							}elseif(strcmp(mb_strtolower($args[0]), "all") == 0){
			...
			case "pesummon":
				if(($sender instanceof Player and count($args) >= 1 and count($args) <= 3) or (!$sender instanceof Player and count($args) > 1)){
					...
					$isBaby = false;
					if(count($args) == 3){
						$isBaby = strcmp(mb_strtolower($args[2]), "true") == 0;
					}
					foreach($this->getServer()->getOnlinePlayers() as $player){
						if(strcasecmp($player->getName(), $playerName) == 0){
							// find a mob with the name issued
							$mobName = mb_strtolower($args[0]);
							foreach(self::$registeredClasses as $registeredClass){
								if(strcmp($mobName, mb_strtolower($this->getShortClassName($registeredClass))) == 0){
