...
class Chicken extends WalkingAnimal implements IntfCanBreed, IntfCanInteract, IntfCanPanic{
	...
	public function getDrops() : array{
		...
		if($this->isLootDropAllowed()){
			...
			if($this->breedableClass != null && !$this->breedableClass->isBaby()){
				array_push($drops, Item::get(Item::FEATHER, 0, \kenygamer\Core\Main::mt_rand(0, 2)));
				if($this->isOnFire()){
	...
	public function entityBaseTick(int $tickDiff = 1) : bool{
		if($this->dropEggTime === 0){
			$this->dropEggTime = \kenygamer\Core\Main::mt_rand(self::DROP_EGG_DELAY_MIN, self::DROP_EGG_DELAY_MAX);
		}
