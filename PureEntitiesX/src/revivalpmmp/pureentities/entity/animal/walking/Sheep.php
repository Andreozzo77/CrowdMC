...
class Sheep extends WalkingAnimal implements IntfCanBreed, IntfCanInteract, IntfShearable, IntfCanPanic{
	...
	public static function getRandomColor() : int{
		...
		$arr = explode(" ", $rand);
		return intval($arr[\kenygamer\Core\Main::mt_rand(0, count($arr) - 1)]);
	}
	...
	public function getDrops() : array{
		...
		if($this->isLootDropAllowed() and !$this->isSheared() && !$this->getBreedingComponent()->isBaby()){
			...
			if($this->isOnFire()){
				array_push($drops, Item::get(Item::MUTTON_COOKED, 0, \kenygamer\Core\Main::mt_rand(1, 2)));
			}else{
				array_push($drops, Item::get(Item::MUTTON_RAW, 0, \kenygamer\Core\Main::mt_rand(1, 2)));
			}
		}
		return $drops;
	}
