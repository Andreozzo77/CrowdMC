...
class Rabbit extends WalkingAnimal implements IntfCanBreed, IntfCanInteract, IntfCanPanic{
	...
	public function getDrops() : array{
		...
		if($this->isLootDropAllowed()){
			array_push($drops, Item::get(Item::RABBIT_HIDE, 0, \kenygamer\Core\Main::mt_rand(0, 1)));
			if($this->isOnFire()){
				array_push($drops, Item::get(Item::COOKED_RABBIT, 0, \kenygamer\Core\Main::mt_rand(0, 1)));
			}else{
				array_push($drops, Item::get(Item::RAW_RABBIT, 0, \kenygamer\Core\Main::mt_rand(0, 1)));
			}

			if(\kenygamer\Core\Main::mt_rand(0, 100) <= 10){ // at 10 percent chance, rabbits drop rabbit's foot
	...
	public function updateXpDropAmount() : void{
		if($this->getBreedingComponent()->checkInLove()){
			$this->xpDropAmount = \kenygamer\Core\Main::mt_rand(1, 4);
		}
		if(!$this->getBreedingComponent()->isBaby()){
			$this->xpDropAmount = \kenygamer\Core\Main::mt_rand(1, 3);
		}
	}
}
