...
class Cow extends WalkingAnimal implements IntfCanBreed, IntfCanInteract, IntfCanPanic{
	...
	public function getDrops() : array{
		...
		if($this->isLootDropAllowed()){
			array_push($drops, Item::get(Item::LEATHER, 0, \kenygamer\Core\Main::mt_rand(0, 2)));
			if($this->isOnFire()){
				array_push($drops, Item::get(Item::COOKED_BEEF, 0, \kenygamer\Core\Main::mt_rand(1, 3)));
			}else{
				array_push($drops, Item::get(Item::RAW_BEEF, 0, \kenygamer\Core\Main::mt_rand(1, 3)));
			}
		}
		return $drops;
	}
	...
	public function updateXpDropAmount() : void{
		if($this->getBreedingComponent()->checkInLove()){
			$this->xpDropAmount = \kenygamer\Core\Main::mt_rand(1, 7);
		}
