...
class Skeleton extends WalkingMonster implements IntfCanEquip, ProjectileSource{
	...
	public function attackEntity(Entity $player){
		...
		if($this->attackDelay > 30 && \kenygamer\Core\Main::mt_rand(1, 32) < 4 && $this->distanceSquared($player) <= 55){
			$this->attackDelay = 0;
			...
			$player->attack($damage = new EntityDamageByEntityEvent($ev->getProjectile(), $player, EntityDamageEvent::CAUSE_PROJECTILE, 4));
			$damage->call();
	...
	public function getDrops() : array{
		...
		if($this->isLootDropAllowed()){
			array_push($drops, Item::get(Item::ARROW, 0, \kenygamer\Core\Main::mt_rand(0, 2)));
			array_push($drops, Item::get(Item::BONE, 0, \kenygamer\Core\Main::mt_rand(0, 2)));
		}
		return $drops;
	}
