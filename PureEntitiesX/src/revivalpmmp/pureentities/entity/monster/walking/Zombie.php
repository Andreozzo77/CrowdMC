...
class Zombie extends WalkingMonster implements IntfCanEquip, IntfCanBreed, Monster{
	...
	public function entityBaseTick(int $tickDiff = 1) : bool{
		...
		if(!$this->isOnFire() && $this->getLevel()->getFullLightAt((int) $this->x, (int) $this->y, (int) $this->z) === 15 && !$this->isUnderwater()){
			$this->setOnFire(100);
		}
	...
	public function getDrops() : array{
		...
		if($this->isLootDropAllowed()){
			array_push($drops, Item::get(Item::ROTTEN_FLESH, 0, \kenygamer\Core\Main::mt_rand(0, 2)));
			// 2.5 percent chance of dropping one of these items.
			if(\kenygamer\Core\Main::mt_rand(1, 1000) % 25 == 0){
				switch(\kenygamer\Core\Main::mt_rand(1, 3)){
					case 1:
