...
class Blaze extends FlyingMonster implements ProjectileSource{
	...
	public function checkTarget(bool $checkSkip = true){
		if(($checkSkip and $this->isCheckTargetAllowedBySkip()) or !$checkSkip){
			...
			if($this->moveTime <= 0 or !$this->getBaseTarget() instanceof Vector3){
				$x = \kenygamer\Core\Main::mt_rand(20, 100);
				$z = \kenygamer\Core\Main::mt_rand(20, 100);
				$this->moveTime = \kenygamer\Core\Main::mt_rand(300, 1200);
				$this->setBaseTarget($this->add(\kenygamer\Core\Main::mt_rand(0, 1) ? $x : -$x, 0, \kenygamer\Core\Main::mt_rand(0, 1) ? $z : -$z));
			}
		}
	}
	...
	public function attackEntity(Entity $player){
		...
		if($this->attackDelay > 20 && $this->distance($player) <= 18){
			...
			$f = 1.2;
			$yaw = $this->yaw + \kenygamer\Core\Main::mt_rand(-220, 220) / 10;
			$pitch = $this->pitch + \kenygamer\Core\Main::mt_rand(-120, 120) / 10;
			$pos = new Location(
	...
	public function getDrops() : array{
		if($this->isLootDropAllowed()){
			return [Item::get(Item::BLAZE_ROD, 0, \kenygamer\Core\Main::mt_rand(0, 1))];
		}else{
			return [];
		}
	}
