	...
	protected function checkTarget(bool $checkSkip = true){
		if(($checkSkip and $this->isCheckTargetAllowedBySkip()) or !$checkSkip){
			...
			if($this->moveTime <= 0 or !$this->getBaseTarget() instanceof Vector3){
				...
				if($this->y > $maxY){
					$y = \kenygamer\Core\Main::mt_rand(-12, -4);
				}else{
					$y = \kenygamer\Core\Main::mt_rand(-10, 10);
				}
				$this->moveTime = \kenygamer\Core\Main::mt_rand(300, 1200);
				$this->setBaseTarget($this->add(\kenygamer\Core\Main::mt_rand(0, 1) ? $x : -$x, $y, \kenygamer\Core\Main::mt_rand(0, 1) ? $z : -$z));
			}
		}
	}
