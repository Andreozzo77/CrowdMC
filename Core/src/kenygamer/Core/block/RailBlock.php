...
use kenygamer\Core\util\Orientation;

class RailBlock extends PMRail{
	...
	private function checkRailsAround(array $faces){
		...
		foreach($faces as $side){
			...
			if($up instanceof RailBlock){
				$result[$side] = $up;
			}
			if($down instanceof RailBlock){
				$result[$side] = $down;
			}
	 ...
	 */
	private function connectTo(RailBlock $other, int $face): Orientation{
	 ...
	 */
	public function connectMultiples(RailBlock $rail1, int $face1, RailBlock $rail2, int $face2): Orientation{
