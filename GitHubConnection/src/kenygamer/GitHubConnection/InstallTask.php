...
final class InstallTask extends AsyncTask{
	...
	public function onRun() : void{
		...
		foreach($repositories as $repositoryObj){
			...
			if(file_exists($pluginPath . '/filemap.yml')){
				echo Terminal::$COLOR_YELLOW . "[GitHubConnection] " . Terminal::$COLOR_AQUA . " Parsing filemap.yml..." . PHP_EOL;
				$cfg = new Config($pluginPath . '/filemap.yml', Config::YAML);
				foreach($cfg->getAll() as $source => $dest){
					//$source = str_replace("%serverPath%", \pocketmine\PATH, $source);
					//$source = str_replace("environment_files/", "environment_files/production/", $source);
					if(!class_exists(Core::class) || Core::$snapshot === ""){
						$source = str_replace("%repoPath%", "%serverPath%plugins/", $source);
						$dest = str_replace("%repoPath%", "%serverPath%plugins/", $dest); //This is not used tho
					}
					$source = str_replace(["%serverPath%", "%repoPath%"], [\pocketmine\PATH, $pluginPath . $repo . '/'], $source);
					$dest = str_replace(["%serverPath%", "%repoPath%"], [\pocketmine\PATH, $pluginPath . $repo . '/'], $dest);
					if(!(copy($source, $dest))){
