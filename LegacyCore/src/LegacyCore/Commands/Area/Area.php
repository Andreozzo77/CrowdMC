...
namespace LegacyCore\Commands\Area;
...
class Area extends PluginCommand{
    ...
    public function execute(CommandSender $sender, string $alias, array $args): bool{
		...
		$playerName = mb_strtolower($sender->getName());
		...
		switch($action) {
				...
				if($sender->hasPermission("core.command.area")){
					if(isset($args[1])){
						if(isset($this->firstPosition[$playerName], $this->secondPosition[$playerName])){
							if(!isset($this->areas[mb_strtolower($args[1])])){
								new ProtectedArea(mb_strtolower($args[1]), ["edit" => true, "god" => false, "touch" => true], $this->firstPosition[$playerName], $this->secondPosition[$playerName], $sender->getLevel()->getFolderName(), [$playerName], $this);
								$this->saveAreas();
			...
			case "here":
				if($sender->hasPermission("core.command.area")){
					$o = "";
					foreach($this->areas as $area){
						if($area->contains($sender->getPosition(), $sender->getLevel()->getFolderName()) && $area->getWhitelist() !== null){
			...
			case "tp":
				...
				if($sender->hasPermission("core.command.area")){
					$area = $this->areas[mb_strtolower($args[1])] ?? null;
			...
			case "flag":
				if($sender->hasPermission("core.command.area")){
					if(isset($args[1])){
						if(isset($this->areas[mb_strtolower($args[1])])){
							$area = $this->areas[mb_strtolower($args[1])];
							if(isset($args[2])){
								if(isset($area->flags[mb_strtolower($args[2])])){
									$flag = mb_strtolower($args[2]);
									if(isset($args[3])){
										$mode = mb_strtolower($args[3]);
									...
									$o = TextFormat::RED . "Flag not found. (Flags: edit, god, touch, shooting)";
								}
							}else{
								$o = TextFormat::RED . "Please specify a flag. (Flags: edit, god, touch, shooting)";
							}
						}else{
			...
			case "whitelist":
				if($sender->hasPermission("core.command.area")){
					if(isset($args[1], $this->areas[mb_strtolower($args[1])])){
						$area = $this->areas[mb_strtolower($args[1])];
						if(isset($args[2])){
							$action = mb_strtolower($args[2]);
							switch($action){
								case "add":
									$w = ($this->plugin->getServer()->getPlayer($args[3]) instanceof Player ? mb_strtolower($this->plugin->getServer()->getPlayer($args[3])->getName()) : mb_strtolower($args[3]));
	...
	public function canGetHurt(Entity $entity) : bool{
		$o = true;
		$default = (isset($this->levels[$entity->getLevel()->getFolderName()]) ? $this->levels[$entity->getLevel()->getFolderName()]["God"] : $this->god);
		if ($default) {
			$o = false;
		}
		foreach($this->areas as $area) {
			if ($area->contains(new Vector3($entity->getX(), $entity->getY(), $entity->getZ()), $entity->getLevel()->getFolderName())) {
	...
	public function canEdit(Player $player, Position $position) : bool{
		...
		$o = true;
		$g = (isset($this->levels[$position->getLevel()->getFolderName()]) ? $this->levels[$position->getLevel()->getFolderName()]["Edit"] : $this->edit);
		if ($g) {
			$o = false;
		}
		foreach($this->areas as $area) {
			if ($area->contains($position, $position->getLevel()->getFolderName())) {
				//Protect areas and exempt ladders of mines
				if ($area->getFlag("edit") || (($area->getName() === "normalmine" xor $area->getName() === "premiummine") && $position->getLevel()->getBlockIdAt($position->x, $position->y, $position->z) === 65)){
					$o = false;
				}
				if ($area->isWhitelisted(mb_strtolower($player->getName()))) {
					$o = true;
					break;
				}
	...
	public function canTouch(Player $player, Position $position) : bool{
		...
		$o = true;
		$default = (isset($this->levels[$position->getLevel()->getFolderName()]) ? $this->levels[$position->getLevel()->getFolderName()]["Touch"] : $this->touch);
		if($default){
			$o = false;
		}
		foreach($this->areas as $area) {
			if ($area->contains(new Vector3($position->getX(), $position->getY(), $position->getZ()), $position->getLevel()->getFolderName())) {
