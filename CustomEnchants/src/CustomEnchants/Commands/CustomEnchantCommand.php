...
namespace CustomEnchants\Commands;
...
class CustomEnchantCommand extends PluginCommand
{
    ...
    public function __construct($name, Main $plugin)
    {
        ...
        $this->setUsage("/customenchant <enchant|help|info|list>");
        $this->setAliases(["ce"]);
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): bool
    {
        ...
        if ($plugin instanceof Main) {
            $forms = $sender instanceof Player;
            if (count($args) < 1) {
                if ($forms) {
                    $this->formMenu($sender);
                    return true;
                }
                $sender->sendMessage(TextFormat::RED . "Usage: /customenchant <enchant|help|info|list>");
                return false;
            }
            switch ($args[0]) {
                case "enchant":
                    if (!$sender->hasPermission("customenchants.command.ce.enchant")) {
                        $this->error($sender, TextFormat::RED . "You do not have permission use this command");
                        return false;
                    }
                    if (count($args) < 2) {
                        if ($forms) {
                    ...
                    break;
                case "help":
                    if (!$sender->hasPermission("customenchants.command.ce.help")) {
                        $this->error($sender, TextFormat::RED . "You do not have permission use this command");
                        return false;
                    }
                    if ($forms) {
                ...
                case "info":
                    ...
                    break;
                case "list":
                    if (!$sender->hasPermission("customenchants.command.ce.list")) {
                        $this->error($sender, TextFormat::RED . "You do not have permission use this command");
                        return false;
                    }
                    if($forms){
                    ...
                    if (isset($data[0])) {
                        switch ($data[0]) {
                            case 0:
                                ...
                                break;
                            case 1:
                                if (!$player->hasPermission("customenchants.command.ce.help")) {
                                    $this->errorForm($player, TextFormat::RED . "You do not have permission use this command");
                                    return false;
                                }
                                $this->helpForm($player);
                                break;
                            case 2:
                                if (!$player->hasPermission("customenchants.command.ce.info")) {
                                    $this->errorForm($player, TextFormat::RED . "You do not have permission use this command");
                                    return false;
                                }
                                ...
                                break;
                            case 3:
                                if (!$player->hasPermission("customenchants.command.ce.list")) {
                                    $this->errorForm($player, TextFormat::RED . "You do not have permission use this command");
                                    return false;
                                }
    ...
    public function enchant(CommandSender $sender, $enchantment, $level, $target)
    {
        ...
        if ($plugin instanceof Main) {
            ...
            if ($plugin->canEnchant($target, $enchantment)){
