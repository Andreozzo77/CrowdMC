...
class BlockStringParser{
    ...
    public static function isValid(string $str): bool {
        self::ensureMap();
        if(is_numeric($str) || isset(self::$blockMap[mb_strtoupper($str)])){
            return true;
        }

        $arr = explode(":", $str);
        if(count($arr) === 2 && is_numeric($arr[1])){
            return is_numeric($arr[0]) || isset(self::$blockMap[mb_strtoupper($arr[0])]);
        }
    ...
    public static function parse(string $str): array{
        ...
        elseif(isset(self::$blockMap[mb_strtoupper($str)])){
            return [self::$blockMap[mb_strtoupper($str)], 0];
        }
        ...
        if (count($arr) === 2 && is_numeric($arr[1])) {
            ...
            elseif(isset(self::$blockMap[mb_strtoupper($arr[0])])){
                return [self::$blockMap[mb_strtoupper($arr[0])], $arr[1]];
            }
        }
