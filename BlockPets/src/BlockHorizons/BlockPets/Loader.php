...
class Loader extends PluginBase {
	...
	//Pets: Must not be any of Spawner mobs or they will auto-close after mismatching with BlockPets.
	...
	private function selectDatabase(): bool {
		switch(mb_strtolower($this->getBlockPetsConfig()->getDatabase())) {
			default:
			case "mysql":
	...
	public function petExists(string $entityName): bool {
		foreach(self::PETS as $pet) {
			if(mb_strtolower($pet) === mb_strtolower($entityName)) {
				return true;
			}
		}
		return false;
	...
	public function createPet(string $entityName, Player $player, string $name, float $scale = 1.0, bool $isBaby = false, int $level = 1, int $levelPoints = 0, bool $chested = false, bool $isVisible = true, ?string $inventory = null): ?BasePet {
		...
		if($entity instanceof BasePet) {
			...
			$this->playerPets[$player->getLowerCaseName()][mb_strtolower($entity->getPetName())] = $entity;
			return $entity;
		}
		return null;
	}
	...
	public function getPetByName(string $name, ?string $player = null): ?BasePet {
		...
		if($player !== null) {
			return $this->playerPets[mb_strtolower($player)][$name] ?? null;
		}
	...
	public function removePet(BasePet $pet, bool $close = true): void {
		...
		unset($this->playerPets[mb_strtolower($pet->getPetOwnerName())][mb_strtolower($pet->getPetName())]);
	}
