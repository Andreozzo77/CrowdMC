...
abstract class BasePet extends Creature implements Rideable {
	...
	protected function sendSpawnPacket(Player $player): void {
		...
		$pk->type = AddEntityPacket::LEGACY_ID_MAP_BC[static::NETWORK_ID];
	...
	public function attack(EntityDamageEvent $source): void {
			...
			if($player instanceof Player) {
				...
				if($hand instanceof Food) {
					...
					if($this->getLoader()->getBlockPetsConfig()->giveExperienceWhenFed()) {
						$this->addPetLevelPoints(intval($nutrition / 40 * LevelCalculator::getRequiredLevelPoints($this->getPetLevel())));
					}
					...
					if($player->isSneaking() && $this->canRide) {
		...
		if($this->calculator !== null && $this->getPetOwner() !== ""){
			$this->calculator->updateNameTag();
			parent::attack($source);
		}
	}
	...
	protected function initEntity(): void {
		if($this->namedtag->getString("petName", "") === ""){
			return;
		}
		parent::initEntity();
		...
		if($this instanceof EnderDragonPet) {
			$this->rider_seatpos = new Vector3(0, 3.65 + $scale, -1.7);
	...
	protected function checkUpdateRequirements(): bool {
		...
		if($this->namedtag->getString("petOwner", "") === "" || $this->getPetOwner()->isClosed()){
			$this->rider = null;
			$this->riding = false;
			$this->despawnFromAll();
			$this->setDormant();
	...
	public function addLink(Entity $entity, int $type): void {
		...
		switch($type) {
			...
			case self::LINK_RIDING:
				$link = new EntityLink($entity->getId(), $this->getId(), self::STATE_SITTING, true, true);
				...
				if($entity instanceof Player) {
					...
					$link_2 = new EntityLink($entity->getId(), 0, self::STATE_SITTING, true, true);
		...
		switch($type) {
			case self::LINK_RIDER:
				...
				if($entity instanceof Player) {
					...
					$link_2 = new EntityLink($entity->getId(), 0, self::STATE_STANDING, true, true);
				...
				break;
			case self::LINK_RIDING:
				$link = new EntityLink($entity->getId(), $this->getId(), self::STATE_STANDING, true, true);
