...
abstract class HoveringPet extends IrasciblePet {
	...
	public function doRidingMovement(float $motionX, float $motionZ): void {
		...
		if($this instanceof EnderDragonPet){
			$this->move($finalMotionX * 7, $this->motion->y * 7, $finalMotionZ * 7);
		}else{
		...
		if($this->chunk === null or ($this->chunk->getX() !== $chunkX or $this->chunk->getZ() !== $chunkZ)){
			...
			if(!$this->justCreated){
				...
				foreach($newChunk as $player){
					$this->spawnTo($player);
					echo __CLASS__ . " #3" . PHP_EOL;
				}
			}

			if($this->chunk === null){
				echo __CLASS__ . " #4" . PHP_EOL;
				return;
			}

			$this->chunk->addEntity($this);
			echo __CLASS__ . " #5" . PHP_EOL;
		}
	}
