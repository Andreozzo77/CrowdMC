...
abstract class Living extends Entity implements Damageable{
	...
	public function applyDamageModifiers(EntityDamageEvent $source) : void{
		...
		$source->setModifier(-$source->getFinalDamage() * min(ceil(min($totalEpf, 25) * (\kenygamer\Core\Main::mt_rand(50, 100) / 100)), 20) * 0.04, EntityDamageEvent::MODIFIER_ARMOR_ENCHANTMENTS);
		...
		if($source instanceof EntityDamageByEntityEvent){
			...
			foreach($this->armorInventory->getContents() as $k => $item){
				if($item instanceof Armor and ($thornsLevel = $item->getEnchantmentLevel(Enchantment::THORNS)) > 0){
					if(\kenygamer\Core\Main::mt_rand(0, 99) < $thornsLevel * 15){
						$this->damageItem($item, 3);
						$damage += ($thornsLevel > 10 ? $thornsLevel - 10 : 1 + \kenygamer\Core\Main::mt_rand(0, 3));
					}else{
	...
	public function attack(EntityDamageEvent $source) : void{
		if($this instanceof \pocketmine\inventory\InventoryHolder && $this->getInventory() === null){
			return;
		}
	...
	public function knockBack(Entity $attacker, float $damage, float $x, float $z, float $base = 0.4) : void{
		...
		if(\kenygamer\Core\Main::mt_rand() / mt_getrandmax() > $this->getAttributeMap()->getAttribute(Attribute::KNOCKBACK_RESISTANCE)->getValue()){
			$f = 1 / $f;
